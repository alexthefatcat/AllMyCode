# -*- coding: utf-8 -*-
"""Created on Wed May  1 13:10:54 2019@author: milroa1
https://github.com/saikatbsk/DeepDream/blob/master/deep_dream.py
##################################################################
#                         Deep Dream                             #
##################################################################          """
...
### TODO: use argparse
base_image_path = 'images/source/CheshireCat.jpg'
result_prefix   = 'images/dreams/CheshireCat/dreamy/CheshireCat'
...
settings_preset = { 'dreamy': {'features'   : { 'block5_conv1': 0.05,
                                                'block5_conv2': 0.02},
                                'continuity': 0.1,
                                'dream_l2'  : 0.02,
                                'jitter'    : 0    },}
...
def seperate_f_outputs_functions(outs):
    loss_value = outs[0]
    if len(outs[1:]) == 1:
...
def create_loss(model,settings=settings):
    def remove_border(x_,shape):
        return x[:, 2: shape[1]-2, 2: shape[2]-2, :]
    # define the loss
    loss = K.variable(0.)
    ...
    for layer_name in settings['features']:
        ...
        loss -= coeff * K.sum(K.square( remove_border(x, shape) )) / np.prod(shape[1:])
    ...
    # Three losses : mse of output of each layer, mse(img-shifted img), reduce high pizel values
    return loss
...
loss = create_loss(model)# Three losses : mse of output of each layer, mse(img-shifted img), reduce high pizel values
...
f_outputs = K.function([dream], outputs) # outputs = [loss, *grads]
# basically scipy tries to minimze x when input into this function
...
def eval_loss_and_grads(x):
    ...
    outs = f_outputs([x])
    loss_value, grad_values = seperate_f_outputs_functions(outs)
    return loss_value, grad_values
...
class Evaluator(object): # This is just to connect the loss and the grads
    def __init__(self):
...
for i in range(20):
    ...
    #      evaluator.loss,  evaluator.grads  :::  loss_value, grad_values = eval_loss_and_grads(x)
