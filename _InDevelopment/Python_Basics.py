# -*- coding: utf-8 -*-
"""Created on Thu Feb 28 11:41:18 2019@author: milroa1"""
# The quick brown fox jumps over the lazy dog of python code
alphabet = "".join(sorted(set(list("the quick brown fox jumps over the lazy dog")))).replace(" ","")



even_nos  = [12,2,4,18,6,10,14,16,20,8]
intergers = list(range(1,21))
intergers_that_in_even_nos = [i in even_nos for i in intergers]

are_all_intergers_in_even_nos  = all(intergers_that_in_even_nos)
are_any_intergers_in_even_nos  = any(intergers_that_in_even_nos)
sum_intergers_that_in_even_nos = sum(intergers_that_in_even_nos)


    

pp=list(reversed(even_nos))

for i1,(i2,even_no) in enumerate(list(reversed(list(enumerate(pp))))): 
    print(i1,i2,even_no)
    
def enum_rev(lis):
    return list(reversed(list(enumerate(even_nos))))    

for i1, (i2,even_no) in enum_rev(enum_rev(even_nos)):    
    print(i1,i2,even_no)
